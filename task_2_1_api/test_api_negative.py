...
def test_create_item_negative_price(api_client, unique_seller_id):
    """Проверяем, что отрицательная цена отклоняется."""
...
@allure.title("TC-015: Отрицательная статистика")
@pytest.mark.negative
def test_create_item_negative_statistics(api_client, unique_seller_id):
    """Проверяем, что отрицательная статистика отклоняется."""
    response = api_client.create_test_item(
        seller_id=unique_seller_id,
        name="Тест",
        price=100,
        statistics={"likes": -5, "viewCount": 0, "contacts": 0},
    )
    assert response.status_code in [400, 422]
...
def test_get_item_not_found(api_client):
    """Проверяем, что 404 для несуществующего ID."""
...
def test_get_items_by_seller_negative_id(api_client):
    """Проверяем, что отрицательный sellerID отклоняется."""
...
def test_get_statistics_not_found(api_client):
    """Проверяем 404 для статистики несуществующего объявления."""
